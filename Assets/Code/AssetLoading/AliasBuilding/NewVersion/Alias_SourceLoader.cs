...
public static class Alias_SourceLoader {
    ...
    static ProcessingImage[,,] loadMip(int mipLevel, string filename, SplicingObject_XML ob, Texture2D unitDefinition)
    {
        ...
        return processTexture(imageTexture, ob, unitDefinition, (resolution >> mipLevel));
    }
    ...
    /// </summary>
    static ProcessingImage[,,] processTexture(Texture2D tex, SplicingObject_XML ob, Texture2D unit, int res)
    {
            ...
            for (int j = 0; j < ob.depth; j++)
            {
                for (int k = 0; k < ob.trueHeight; k++)
                {
                    if (isVoid(i, j, k, ob.trueHeight))
                        ...
                        ret[i, j, k] = getImage(tex, new Iso(i,j,k), originX, originY, ob.name, res, unit);
                }
            }
        }
